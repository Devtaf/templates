( function ( $ ) {
    ...
    $( document ).ready( function () {
         ...
         */
        $( function () {
            let selector = $( '.rhea-sticky-container' );
            // Element must exist
            if ( ! selector.length ) {
                return;
            }

            // Create placeholder to prevent jump
            let placeholder = jQuery( '<div class="rhea-sticky-placeholder"></div>' );
            placeholder.insertAfter( selector ).hide();

            let offsetTop = selector.offset().top;
            let height    = selector.outerHeight();

            const adminBar          = jQuery( '#wpadminbar' );
            const getAdminBarHeight = () => adminBar.length ? adminBar.outerHeight() : 0;

            jQuery( window ).on( 'scroll resize', function () {
                let scrollTop      = jQuery( window ).scrollTop();
                let adminBarHeight = getAdminBarHeight();

                if ( scrollTop > offsetTop - adminBarHeight ) {
                    placeholder.height( height ).show();
                    selector.addClass( 'is-sticky' ).css( {
                        top : adminBarHeight + 'px'
                    } );
                } else {
                    placeholder.hide();
                    selector.removeClass( 'is-sticky' ).css( { top : '' } );
                }
            } );
        } );
